...
angular.module('college_home')
    .controller('teacherDetailCtrl', function($state,$scope, $rootScope,commonUtil,teacherService) {
        var vm = $scope.vm = {};
        vm.id = $state.params.id;

        if (vm.id) {
            teacherService.getModule(vm.id).then(function(res) {
                if (res.data.code == 0) {
                    vm.data = res.data.data.teacher;
                }else{
